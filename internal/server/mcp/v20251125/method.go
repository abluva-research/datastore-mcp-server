...
func toolsCallHandler(ctx context.Context, id jsonrpc.RequestId, resourceMgr *resources.ResourceManager, body []byte, header http.Header) (any, error) {
	...
	// Geo-fence enforcement: check if the pseudo key's allowed region matches the client's location
	if checker, ok := geofence.CheckerFromContext(ctx); ok {
		vi, _ := pseudokey.FromContext(ctx)
		isStdio := geofence.IsStdioFromContext(ctx)

		// Extract optional client IP/region from arguments (HTTP mode clients send these)
